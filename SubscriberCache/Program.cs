...
using SubscriberCache.Messaging;
using SubscriberCache.Options;
using SubscriberCache.Services;
using StackExchange.Redis;

var builder = WebApplication.CreateBuilder(args);

builder.Services.Configure<SubscriberCacheConfiguration>(
    builder.Configuration.GetSection(SubscriberCacheConfiguration.SectionName));
...
{
    var redisConnectionString = builder.Configuration.GetValue<string>("Redis:ConnectionString") ?? "localhost:6379";
    return new SubscriberCacheConfiguration
    {
...
builder.Services.AddSingleton<SubscriberCacheService>();
//builder.Services.AddSingleton<ArticleTracker>();

builder.Services.AddHostedService<RabbitMqSubscriberConsumerHostedService>();
...
app.MapGet("/stats/cache", async (SubscriberCacheService cache) =>
{
